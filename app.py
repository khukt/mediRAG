...
def build_relevant_context(question, medicines):
    ...
    for drug in medicines:
        ...
        if "paracetamol" in keywords or "ibuprofen" in keywords or any(kw in drug['generic_name'].lower() for kw in keywords):
            ...
            drug_info.append(f"Dosage Forms: {', '.join([f'{d['form']} ({', '.join(d['strengths'])})' for d in drug['dosage_forms']])}\n")
            ...
            drug_info.append(f"Interactions: " + "; ".join([f"{i['drug']}: {i['description']}" for i in drug['interactions']]) + "\n")
